...
class Controller_User extends Controller_BaseController {
     ...
     * 获取我的优惠券
     */
    public function action_coupons($id = 0){
        $item = \Model_PeopleCoupon::query()
            ->related('coupon')
            ->where(['user_id' => $id, 'status' => 'NONE'])
            ->where('coupon.start', '<', time())
            ->where('coupon.end', '>', time())
            ->where('coupon.not_allowed_date', 'NOT IN', [date('w'), date('Y-m-d')]);

        if(\Input::get('money', false)){
            $item->where('coupon.quota', '<=', \Input::get('money'));
        }
        $coupons = $item->get();

        $items = [];
        foreach($coupons as $coupon){
            if(in_array(date('w'), explode(',', $coupon->coupon->not_allowed_date))
                || in_array(date('Y-m-d'), explode(',', $coupon->coupon->not_allowed_date))){
                continue;
            }
            $item = $coupon->to_array();
            $item['coupon'] = $coupon->coupon->to_array();
